...
import { flattenTree, fmtRaw, fmtMoney } from "@/lib/balanceteCalc";
            ...
            {rows.map((r, i) => {
              ...
              return (
                <tr key={i} className={isLeaf ? "" : "bg-slate-50/60"}>
                  ...
                  <td className="px-2 py-2 text-right text-slate-500">{fmtRaw(r.saldoAnteriorRaw)}</td>
                  <td className="px-2 py-2 text-right text-red-600">{fmtMoney(r.debito)}</td>
                  <td className="px-2 py-2 text-right text-blue-600">{fmtMoney(r.credito)}</td>
                  <td className="px-2 py-2 text-right font-medium text-slate-900">{fmtRaw(r.saldoAtualRaw)}</td>
                </tr>
              );
            })}
          </tbody>
